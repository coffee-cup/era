...
The current implementation keeps this cache workspace-scoped and persists it as a redb ordered key-value database under `.era/workspaces/<workspace-id>/cache/capture-v2.redb`. It records path-local file fingerprints, blob IDs, and stored directory tree entries. Full captures and status scans still inspect included filesystem paths, but hinted watch captures can use indexed point lookups, point writes, and prefix invalidation instead of parsing or rewriting the whole cache. Cache commits use non-fsync durability because the cache is disposable; missing or corrupt cache files rebuild from the working tree and object store. Shared objects remain global; per-path fingerprints remain local to the materialized workspace.
...
The current user-facing form is foreground `era watch`, not a daemon started by `era init`. It debounces watcher events, invalidates affected cache entries, and asks the materializer to capture from the dirty path hints. Periodic reconciliation still scans the full working tree to catch missed events. Watchers are imperfect: events can be lost, coalesced, or duplicated, and edge cases differ across platforms. The watch flow treats events as optimization hints, not exclusive ground truth.
