...
- It can watch a working directory and emit filtered filesystem change hints for auto-snapshot loops.
- It keeps a per-materializer in-memory hash cache so long-running watch sessions can reuse unchanged file hashes.
...
- Persistent hash caching remains future materialization work; the current cache is in-memory and scoped to one materializer/workspace.
...
- Automatic snapshot requests capture the current working directory and advance the branch only when the root tree changed.
...
Commands use clean default output with adaptive terminal coloring and a global `--verbose` flag for full object IDs, root tree IDs, timestamps, repository paths, provenance attributes, and capture/materialization/cache stats. `era snap` is the single user-facing "remember this state" command: it captures the current tree and attaches an optional label, defaulting to the current local timestamp formatted like `Jan 1, 2024 11:11:11`.

`era status` compares the working tree to the current saved snapshot and reports either `no changes` or changed paths marked as added (`A`), modified (`M`), deleted (`D`), or type-changed (`T`). `era branch` lists or creates branches at the current saved state. `era switch` saves unsnapped work before switching branches and materializing the target branch. `era restore` saves unsnapped work before restoring a snapshot ID, unique ID prefix, or exact snapshot label into the working tree without moving the current branch pointer. `era watch` runs in the foreground, debounces filesystem events, periodically reconciles the full tree, and creates unlabeled automatic snapshots with structured trigger/workspace/agent provenance.

The CLI is covered by integration tests that run the compiled `era` binary through init/snapshot/status/branch/switch/restore/watch/timeline flows in temporary repositories and verify user-facing errors.
